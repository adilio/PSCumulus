# Cross-Cloud without Crossed Fingers — v2

**Second-pass refinement for PowerShell + DevOps Global Summit 2026.**

This document is the consolidated output of a second-pass revision. It takes the bones of the first rebuild (see `ANALYSIS.md`) and brings the original's voice and warmth back in where the first pass sanded them off. Three directives shaped this pass:

1. **Preserve the spirit.** The original had a tone. The first pass got more rigorous; this pass gets more human again without losing the rigor.
2. **Earn every laugh.** The Summit audience has lived this. Humor comes from recognition — the fourth time you looked up whether it's `--region` or `-Region`, the three different credential file locations — never from performed relatability.
3. **Lead with education.** PSCumulus is the vehicle, not the destination. A person who never installs the module should still leave with a better mental model of why multi-cloud operations are hard and how to think about abstracting across them.

One running constraint: **Microsoft engineers, AWS folks, and Google Cloud folks are all in the room.** The framing is always *"here's the complexity we're all navigating together"* — never *"here's why $CLOUD made your life hard."* Nobody's cloud is the villain. The villain is the inherent difficulty of three independently-evolved systems not lining up.
...
Fifteen slides. One live terminal. One module. No housekeeping slide at the end.

---

## PART ONE — REVISED SLIDE DECK

Each slide lists: **title**, **on-screen content**, and **speaker notes** written as natural spoken language. Humor beats are marked `[laugh beat]` where the speaker should let the line breathe; if the room doesn't laugh, he moves on without flinching.

---

### Slide 1 — Title

**On screen.** *Cross-Cloud without Crossed Fingers — Surviving Azure, AWS, and GCP with PowerShell.* Full name: **Adil Leghari**. `@adilio` · Wiz. Summit 2026 branding.

**Speaker notes.**

> All right. Thanks for being here. This is a talk about three cloud providers, one keyboard, and the specific feeling of typing a command and realizing halfway through that you have no idea which cloud you're actually in.
>
> I'm Adil. I work at Wiz on the solutions engineering side. Before that I did a long stretch of PowerShell and sysadmin work, some of it in places where you find out at 2am — not before — that you were supposed to know all three of these clouds. That was the talk I almost gave, five years ago. Now it's a module, and it's this.

---

### Slide 2 — The Frozen Terminal

**On screen.** A stylized terminal block, monospace, large:
...
Subtitle, small: *The four-second pause.*
...
>
> You know this moment. You're halfway through a command, you're staring at the flag you just typed, and you genuinely cannot remember whether this CLI wants one dash or two. You can't remember whether `--region` has a capital R in this one. You're pretty sure this flag exists. You're not sure it exists *here*. And for about four seconds you just — stop. [laugh beat] And for that beat, you don't know which cloud you're in anymore.
>
> That was me. It's still me sometimes. I once spent ten minutes debugging a `gcloud` command that wasn't working, and it wasn't working because it was an `az` command. [laugh beat] I was bouncing between three clouds, and I didn't feel like I was learning one big system. I felt like I was renting three different brains, and one of them was always out.
>
...
**On screen.** Top line: *The clouds disagree on what a thing* is.
...
Below: *Same question. Three different grammars. IAM is just the loudest example.*

**Speaker notes.**

> Before we get into the module, one thing worth saying out loud, because I think it gets left out of a lot of multi-cloud talks: multi-cloud is hard for a reason, and the reason is not that you are bad at it.
>
> These three clouds were not designed to coexist in anyone's head. They were designed independently, by different companies, at different times, with different first customers and different opinionated answers to fundamental questions. *What is a resource? How is it scoped? Who owns it? How do you grant someone access to it?* Each of these clouds answered those questions thoughtfully — and separately. That's not a bug on anyone's side. That's just what happens when three excellent teams solve the same problem in parallel.
>
> And then we have to operate across all three of them with the same two hands.
>
> The loudest example is identity. AWS expresses access as policy documents. Azure expresses access as role assignments scoped to a hierarchy. GCP expresses access as bindings attached to resources. Those aren't the same thing wearing different clothes. Those are three different grammars for the same English sentence.
>
> Hold that example in your head. I'm coming back to it, because it turns out to be the single most important slide in this talk — and it's not for the reason you probably think.

---

### Slide 4 — Build on What Does Not Move

**On screen.** One large line, center:

> **Fluency is infrastructure.**

Small subtitle: *Build on what does not move.*

**Speaker notes.**

> So if the systems don't agree, I want an anchor that does. For me, the anchor was PowerShell.
>
> Not because PowerShell is objectively best — I genuinely don't believe in "objectively best" for a tool. [laugh beat] If you use bash, we're still friends. If you use Python, we're still friends. If you use a Rust CLI you wrote yourself because you don't trust anybody's tooling, we're *especially* still friends.
>
> PowerShell is the tool I'm most fluent in. My hands already know the shape: verb-noun, pipeline, object out. The cognitive cost of a PowerShell command for me is close to zero. That's what I wanted for cross-cloud work. Not the best tool on paper. The *fluent* tool in my hands.
>
> That's the bet the rest of this talk is about: when the systems underneath you don't hold still, you build on the thing that does. For me that's PowerShell. For you it might be something else, and that's fine — the idea travels. Fluency is infrastructure.

---

### Slide 5 — PSCumulus

**On screen.** Title: *PSCumulus — a thin, honest abstraction for Azure, AWS, and GCP.*

Two-column command list, monospace:
...
Footer: *Same verbs. Same nouns. Same output shape. No pretending the providers are identical.*

**Speaker notes.**

> So here's the module. It's called PSCumulus — because I needed a name, and "cumulus" was a cloud word nobody had taken yet, and "PS" is how every PowerShell module legally has to start. [laugh beat] Brief.
>
> Eleven public commands. Deliberately small. The first pass had about twenty and I cut half of them, and the half I cut is more of the talk than the half that's left.
>
> Two things to flag before we look at code.
>
> First: the noun is always `Cloud<Thing>`. Never `Az`. Never `EC2`. Never `GCP`. The public noun is a normalized concept; the native type lives in metadata.
>
> Second: every read command returns the same output shape, regardless of which cloud it hit. That's the whole bet. Let me show you what that looks like.

---

### Slide 6 — [DEMO] Native vs. Unified

**On screen.** `DEMO` slug top-left. Two code columns side by side.
...
**Speaker notes.** *(The slide is a safety net. The primary canvas is the terminal.)*

> *[Switch to terminal.]*
>
> The three commands on the left, you already know. `Get-AzVM`. `Get-EC2Instance`. `gcloud compute instances list --format=json`. All three of those answer the same human question: *what compute do I have here?* All three of them answer it in completely different output shapes.
>
...
>
> One call, three providers. Under the hood this checks each provider for an existing session, triggers the native login flow if there isn't one — and yes, that's three different login flows stitched together, because there are three different login flows in the world, and I can't fix that with a module. What I *can* do is stop making you remember which one is which. The contexts live side by side. Connecting to one doesn't disconnect the others. This is a small thing and I lost an afternoon to it once, so I fixed it.
>
...
>
> Same command, AWS this time. Same shape.
>
> *[Run: `Get-CloudInstance -Provider GCP -Project contoso-prod`]*
>
> GCP. Same shape. The output doesn't know which cloud it came from until you look at the Provider column. That's the whole point.
...
Below: *Untagged production assets across every connected cloud. One pipeline. Three providers. One output shape to filter against.*

**Speaker notes.**

> *[Back to terminal.]*
>
> This is the beat the whole module is built around. If nothing else lands today, this is the one.
>
> *[Run: `Get-CloudInstance -All`]*
>
> That flag — `-All` — iterates every provider that has stored context, calls each backend, and streams one pipeline of `CloudRecord` objects. I'm not writing three loops. I'm not merging three output shapes with `Select-Object` gymnastics. I'm just getting one stream.
>
> And once you have one stream, you can do this.
>
> *[Run: the three-line pipeline.]*
>
...
>
> The first time this worked end to end, I stared at it for a minute. Then I ran it again, because I was pretty sure I was hallucinating. [laugh beat]
>
> *[Run: `Show-FleetHealth` or a `Group-Object Provider` pipeline.]*
>
...
**On screen.** Table header: `PSCumulus.CloudRecord`.
...
Footer: *Seven safe columns. One honest* Metadata *property for everything else.*

**Speaker notes.**

> *[Back to slides.]*
>
> That shape has a name. `PSCumulus.CloudRecord`. Eight fields.
>
> Seven of these are what you can safely filter and group against across clouds, because the three providers all have a coherent answer for them. A name, a provider, a region, a status, a size-ish thing, a creation timestamp, and a bag of tags. Those translate.
>
> The eighth field is `Metadata`. Metadata is where I put the provider-native stuff that genuinely matters and genuinely does not generalize. Your Azure resource group. Your AWS VPC ID. Your GCP zone. Those are real. They're not lies. They're just not portable, and promoting them into the first seven columns would be lying about what's portable.
>
...
**On screen.** Three stacked lines, increasing contrast:
...
> Quick aside on the naming, because somebody always asks.
>
> I picked `Get-CloudInstance`, not `Get-VM`. Two reasons.
>
> First: `Get-VM` is already owned by the Hyper-V world in PowerShell, and it has been for a long time. I wasn't going to have this module walk in and pretend it owned that noun. Too many people have muscle memory there.
>
> Second: the public noun is a normalized *cloud* concept, not a vendor name. `CloudInstance` tells the truth about what the abstraction is. If I called it `Get-AzureInstance` it would imply Azure is the main character, and it isn't. Same for AWS, same for GCP. Nobody's the main character — that's the shape of the bet.
>
...
Footer: *The test: do the underlying philosophies overlap enough that a normalized answer is still honest?*

**Speaker notes.**

> OK. This is the real content of the talk. If you take one slide home, take this one.
>
> Every command in PSCumulus had to pass a single test. The test was: *do the underlying philosophies behind this concept overlap enough that a normalized answer is still honest?*
>
> For compute, yes. All three clouds agree that a compute instance is a thing that runs, has a name, lives in a region, has a size, has a status. The philosophies line up. `Get-CloudInstance` exists.
>
> For storage: yes, with seams. The billing models differ, the lifecycle policies differ, the consistency guarantees differ — but the operator intent, *what storage exists here*, translates cleanly. `Get-CloudStorage` exists.
>
> For disks, networks, functions, tags — same story. Increasing amounts of seam showing at the edges, but the core concept is close enough that normalizing isn't lying.
>
> And then there's the last row.
>
> IAM. There's a dash where a PSCumulus command would be. That dash is not an oversight. That dash is load-bearing.
...
> If the normalized object would be mostly `Metadata`,
> the abstraction is too weak to deserve a first-class command.

**Speaker notes.**

> Let me make the IAM thing concrete. These three models are each coherent on their own — and each one is the right answer to the problem the team in front of it was solving. That's not criticism of any of them. I'm genuinely impressed by all three. They just don't compose.
>
> AWS expresses access as policy documents. JSON objects that declare what actions are allowed or denied on what resources, attached to users, groups, or roles. Policy-first.
>
...
>
> GCP expresses access as bindings on a resource. Member and role pairs, sometimes conditional, attached at the resource. Resource-first.
>
> Different scoping. Different inheritance. Different mental models. Three good answers to the same question, each correct in its own grammar, none of them translatable into the others without losing information.
>
> If I wrote `Get-CloudPermission` anyway, one of two bad things would happen. Either I'd flatten everything to the least common denominator, and lose the scoping and inheritance behavior that make the answer *useful* in the first place. Or I'd leave the real information in `Metadata`, and the top-level object would be a sincere, friendly wrapper with almost nothing in it. [laugh beat] Either way, I'd be lying — just with different body language.
>
> So there's a rule I use. *If the normalized object would be mostly* Metadata, *the abstraction is too weak to deserve a first-class command.* That's why there's no `Get-CloudPermission`. Instead, three explicit provider-native commands — `Get-AzureRoleAssignment`, `Get-AWSPolicyAttachment`, `Get-GCPIAMBinding`. Three seams, left visible, on purpose.
>
> I think this is the actual skill. Knowing when not to abstract is harder than knowing when to abstract, because when you don't abstract the slide looks empty and you have to defend the empty space. Empty space is a feature here. The module is useful because it refuses to lie about the places where the clouds are genuinely, philosophically different.

---

### Slide 12 — Not Terraform's Job

**On screen.** Two side-by-side cells, equal weight:

- **Terraform** — desired state, provisioning, lifecycle
- **PSCumulus** — operator intent, interactive querying, shared shape

Footer: *Different layer. Not opposition.*

**Speaker notes.**

> Somebody is thinking it, so let's get it out of the way: why not Terraform?
>
> Terraform is great. I use Terraform. Terraform solves a different problem.
>
> Terraform standardizes *infrastructure* — what exists, in what shape, managed as code. It gives you desired state, provisioning, drift correction. PSCumulus standardizes *how a human interacts with* infrastructure once it already exists. It gives you an operational shell with consistent ergonomics across three clouds.
>
> Terraform is not an operational shell. PSCumulus is intentionally behaving like one. These live on different layers. This isn't a turf war. If your question is "what should exist," Terraform. If your question is "what *does* exist, and what shape is it in right now, and can I filter it before I page someone" — that's the shell, and that's where this module lives.

---

### Slide 13 — What This Does Not Do

**On screen.** Clean list:
...
> Before I land this, let me name what PSCumulus doesn't do. The people most likely to ask these questions deserve a straight answer, and I'd rather say it out loud than have it live in a GitHub issue somebody files at 11pm tonight.
>
> There's no cost surface. Cost models across these three clouds are their own multi-hour talk, and it is not this one. [laugh beat]
>
> There's no unified health or status surface — the provider status signals are shaped too differently to compose honestly.
>
> `Start-CloudInstance` and `Stop-CloudInstance` both ship with full `-WhatIf` and `-Confirm` support. Running a bulk pipeline with `-WhatIf` previews every operation across all three clouds before anything executes — this is one of the most important safety features for cross-cloud lifecycle operations.
>
...
**On screen.** One line, centered, large:
...
>
> We spend a lot of time in this field asking what the *right* tool is for a given job. That's a good question, and I don't want to talk anyone out of it. But there's another question I think about more now.
>
> *What's the tool I'll still trust when the job gets weird?*
>
> When you're on call and the environment is half-configured and you cannot remember which cloud you're supposed to be in. When you need to move fast and you genuinely cannot afford a mistake, and you need your hands to already know what to do without you looking anything up. Those are the moments where fluency matters more than optimality. And fluency is built over time, on tools you already know.
>
> For me, that tool was PowerShell. The module you saw is just the map I drew on top of it, with the places marked where the terrain stops cooperating. I hope some of it is useful to you. And if the only thing that travels back home is the *question* — what's the tool you'll still trust when it gets weird — that's fine with me too.
...
Small footer: *Slides and talk track linked in the repo. Thanks.*

**Speaker notes.**

> Repo is at `github.com/adilio/PSCumulus`. Slides and the talk track are in there too, so you don't have to photograph anything. Thanks for listening.

---

## PART TWO — POLISHED 25-MINUTE TALK TRACK

*A continuous, speakable track. Stage directions in [brackets]. `[laugh beat]` is a note to the speaker to let the line land and not step on it — if the room doesn't laugh, keep moving, don't flinch. The whole track is written for the voice: short sentences, contractions, breath marks implied by the paragraph breaks.*
...
- The humor in this version is all rooted in recognition: four-second pauses, wrong CLIs, ten-minute debug sessions, three config file locations. None of it punches down at Azure, AWS, or GCP. If a line ever starts to feel like it's mocking a cloud, cut it live.
...
**Anticipated questions (prep answers in your head).**
...
- *Does `-All` respect some ordering?* — Yes, alphabetical by provider name, deliberately fixed so the output is predictable regardless of connect order.
