    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'Path') {
            ...
            if ($cloudPath.Kind -ne 'Instances') {
                throw [System.ArgumentException]::new(
                    "Restart-CloudInstance is only supported for Instances. Got kind: $($cloudPath.Kind)"
                )
            }
            ...
            $commandMap = @{
                Azure = 'Restart-AzureInstance'
                AWS   = 'Restart-AWSInstance'
                GCP   = 'Restart-GCPInstance'
            }
